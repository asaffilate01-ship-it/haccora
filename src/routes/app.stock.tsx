...
interface Item {
  ...
  qty: number; unit: string; par: number; supplier: string | null;
}
    ...
    <div className="p-6 md:p-10 space-y-8">
      <div className="flex items-end justify-between gap-4 flex-wrap">
        <div>
          ...
          <h1 className="mt-1 text-3xl md:text-4xl">{t("Bestand & Inventur", "Stock & stock-take")}</h1>
          <p className="text-muted-foreground mt-1">
            {t("Bestände live, Par-Level-Warnungen und Inventurabgleich.",
        ...
        {canEdit && (
          <button onClick={() => setShowForm(true)} className="btn-alert-solid text-sm">
            <Plus size={16} className="inline mr-1.5" />{t("Artikel hinzufügen","Add item")}
          </button>
        )}
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <Kpi label={t("Niedrig","Low stock")} value={String(lowCount)} tone={lowCount>0?"warning":"success"} />
        <Kpi label={t("Kategorien","Categories")} value={String(new Set(rows.map(r=>r.category||"")).size)} />
      </div>
      ...
      <div className="surface overflow-hidden">
        {loading ? (
          ...
          <div className="p-10 text-center text-sm text-muted-foreground">
            <ClipboardList size={20} className="inline opacity-40 mr-2" />{t("Noch keine Artikel.","No stock items yet.")}
          </div>
        ) : (
          <>
              ...
              {rows.map((s) => {
                ...
                return (
                  <li key={s.id} className="grid grid-cols-1 md:grid-cols-12 items-center gap-2 px-5 py-3 text-sm">
                    ...
                    <div className="md:col-span-2 text-xs text-muted-foreground">{s.category ?? "—"}</div>
                    ...
                    <div className="md:col-span-2 text-xs flex items-center justify-between">
                      ...
                      {canEdit && <button onClick={() => remove(s.id)} className="text-muted-foreground hover:text-destructive"><Trash2 size={12} /></button>}
                    </div>
                  </li>
                );
              })}
      ...
      {showForm && canEdit && <StockForm onClose={() => setShowForm(false)} onSaved={() => { setShowForm(false); load(); }} t={t} />}
    </div>
  );
}

function StockForm({ onClose, onSaved, t }: { onClose: () => void; onSaved: () => void; t: (de: string, en: string) => string }) {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    await supabase.from("stock_items").insert({
      name, category: category || null, qty: Number(qty)||0, unit, par: Number(par)||0,
      supplier: supplier || null, created_by: user?.id,
    });
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-foreground/40 backdrop-blur-sm p-4">
      <form onSubmit={save} className="surface w-full max-w-md p-6 space-y-3">
        ...
        <F label={t("Name","Name")}><input required value={name} onChange={e=>setName(e.target.value)} className="mt-1 w-full rounded-lg border border-border bg-card px-3 py-2 text-sm" /></F>
        <div className="grid grid-cols-2 gap-3">
          <F label={t("Kategorie","Category")}><input value={category} onChange={e=>setCategory(e.target.value)} className="mt-1 w-full rounded-lg border border-border bg-card px-3 py-2 text-sm" /></F>
          <F label={t("Lieferant","Supplier")}><input value={supplier} onChange={e=>setSupplier(e.target.value)} className="mt-1 w-full rounded-lg border border-border bg-card px-3 py-2 text-sm" /></F>
        </div>
        <div className="grid grid-cols-3 gap-3">
          <F label={t("Menge","Qty")}><input type="number" step="0.01" value={qty} onChange={e=>setQty(e.target.value)} className="mt-1 w-full rounded-lg border border-border bg-card px-3 py-2 text-sm" /></F>
          <F label={t("Einheit","Unit")}><input value={unit} onChange={e=>setUnit(e.target.value)} className="mt-1 w-full rounded-lg border border-border bg-card px-3 py-2 text-sm" /></F>
          <F label="Par"><input type="number" step="0.01" value={par} onChange={e=>setPar(e.target.value)} className="mt-1 w-full rounded-lg border border-border bg-card px-3 py-2 text-sm" /></F>
        </div>
...
function Kpi({ label, value, tone }: { label: string; value: string; tone?: "success" | "warning" }) {
  const cls = tone === "success" ? "text-success" : tone === "warning" ? "text-warning-foreground" : "";
  return (
