  ...
  return (
    <div className="p-6 md:p-10 space-y-8">
      <div>
        ...
        <h1 className="mt-1 text-3xl md:text-4xl">{t("Öffnen · Service · Schließen", "Open · Service · Close")}</h1>
        <p className="text-muted-foreground mt-1">
          {t("Ein geführter Ablauf – Temperaturen, Kontrollen und Reinigung in einer Routine.",
      ...
      <div className="surface p-5 flex items-center gap-5">
        <div className="relative h-16 w-16 shrink-0">
          <svg viewBox="0 0 36 36" className="h-16 w-16 -rotate-90">
            <circle cx="18" cy="18" r="15.9" fill="none" stroke="currentColor" className="text-secondary" strokeWidth="3.5" />
            <circle cx="18" cy="18" r="15.9" fill="none" stroke="currentColor" className="text-primary" strokeWidth="3.5"
            ...
            <button key={p.id} onClick={() => setActivePhase(p.id)}
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-muted-foreground">{doneCount}/{p.steps.length}</div>
                </div>
              ...
              <li key={s.id}>
                <button onClick={() => toggle(phase.id, s)} className="w-full flex items-center gap-4 px-5 py-4 hover:bg-secondary/40 transition text-left">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {s.expect && <div className="text-[11px] text-muted-foreground mt-0.5">{t("Grenzwert", "Limit")}: {s.expect}</div>}
                  </div>
                </button>
              </li>
            );
