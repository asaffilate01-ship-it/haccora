...
import { ClipboardList, CheckCircle2, AlertTriangle, PlusCircle, Award, TrendingUp, Loader2 } from "lucide-react";

export const Route = createFileRoute("/app/audits")({ component: AuditsPage });

interface Audit { id: string; title: string; audit_type: string; score: number | null; status: string; notes: string | null; performed_at: string; performed_by: string | null; }
...
function AuditsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const { data, error } = await supabase.from("audits").select("*").order("performed_at", { ascending: false }).limit(50);
    if (error) setErr(error.message); else setRows((data ?? []) as Audit[]);
    setLoading(false);
  }, []);
  useEffect(() => { load(); }, [load]);

  const submit = async () => {
    if (!form.title.trim()) { setErr(t("Titel ist Pflicht.", "Title is required.")); return; }
    setBusy(true); setErr(null);
    const score = form.score ? parseInt(form.score, 10) : null;
    const { error } = await supabase.from("audits").insert({
      title: form.title, audit_type: form.audit_type,
      score, notes: form.notes || null,
    ...
    <div className="p-6 md:p-10 space-y-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {t("Selbstkontrollen erfassen, Findings dokumentieren, Score verfolgen.",
               "Log self-checks, capture findings, track score over time.")}
          </p>
        </div>
        {canEdit && (
          <button onClick={() => setOpen(o => !o)} className="btn-alert-solid text-sm">
            <PlusCircle size={14} className="inline mr-1.5" />{t("Audit erfassen", "Log audit")}
          </button>
        )}
      </div>

      <div className="grid md:grid-cols-4 gap-4">
        <Kpi label={t("Ø Score", "Avg score")} value={completed.length ? `${avg}%` : "–"} tone="success" icon={Award} />
        <Kpi label={t("Audits gesamt", "Total audits")} value={String(rows.length)} icon={ClipboardList} />
        <Kpi label={t("Kritisch (<70)", "Critical (<70)")} value={String(critical)} tone={critical > 0 ? "destructive" : undefined} icon={AlertTriangle} />
        <Kpi label={t("Abgeschlossen", "Completed")} value={String(completed.length)} tone="success" icon={TrendingUp} />
      </div>

      {open && canEdit && (
        <div className="surface p-5 grid md:grid-cols-4 gap-3">
          <input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder={t("Titel (z. B. Küchenrundgang)", "Title (e.g. Kitchen walkthrough)")} className="md:col-span-2 rounded-lg border border-border bg-card px-3 py-2 text-sm" />
          <select value={form.audit_type} onChange={e => setForm({ ...form, audit_type: e.target.value })} className="rounded-lg border border-border bg-card px-3 py-2 text-sm">
          ...
          <input type="number" min="0" max="100" value={form.score} onChange={e => setForm({ ...form, score: e.target.value })} placeholder={t("Score %", "Score %")} className="rounded-lg border border-border bg-card px-3 py-2 text-sm" />
          <input value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} placeholder={t("Notizen (optional)", "Notes (optional)")} className="md:col-span-3 rounded-lg border border-border bg-card px-3 py-2 text-sm" />
      ...
      <div className="surface overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="p-10 text-center text-sm text-muted-foreground">{t("Noch keine Audits erfasst.", "No audits recorded yet.")}</div>
        ) : (
          <ul className="divide-y divide-border">
            {rows.map(a => (
              <li key={a.id} className="grid grid-cols-1 md:grid-cols-12 items-center px-5 py-3 text-sm gap-2">
                ...
                <div className="md:col-span-2 text-xs uppercase tracking-wider text-muted-foreground">{a.audit_type}</div>
                <div className="md:col-span-3 text-xs text-muted-foreground">{new Date(a.performed_at).toLocaleString(lang === "de" ? "de-DE" : "en-GB")}</div>
                <div className="md:col-span-1 text-right">
                  <span className={`inline-flex text-[10px] font-bold uppercase px-2 py-0.5 rounded ${a.status === "completed" ? "bg-success/15 text-success" : "bg-primary/15 text-primary"}`}>
