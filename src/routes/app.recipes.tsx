...
interface Row {
  ...
  allergens: string[]; cost_eur: number; price_eur: number; flagged: boolean;
}
    ...
    <div className="p-6 md:p-10 space-y-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        {canEdit && (
          <button onClick={() => setShowForm(true)} className="btn-alert-solid text-sm">
            <Plus size={16} className="inline mr-1.5" />{lang === "de" ? "Rezept hinzufügen" : "Add recipe"}
          </button>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {rows.map((r) => {
            const margin = r.price_eur > 0 ? ((r.price_eur - r.cost_eur) / r.price_eur * 100).toFixed(0) : "—";
            return (
              <div key={r.id} className="surface p-5 flex flex-col">
                      ...
                      return (
                        <span key={a}
                          className={`text-[10px] font-medium rounded-full px-2 py-0.5 border ${
                            active ? "bg-accent/25 border-accent text-foreground" : "border-border text-muted-foreground/60"
                          }`}>
                          {t(`allergen.${a}`)}
                        </span>
                      );
                ...
                <div className="mt-5 grid grid-cols-3 gap-2 text-xs">
                  <Cell label={t("recipes.cost")}   value={canCost ? `€${Number(r.cost_eur).toFixed(2)}` : "—"} />
...
function RecipeForm({ onClose, saving, setSaving, onSaved, lang }: {
  onClose: () => void; saving: boolean; setSaving: (b: boolean) => void; onSaved: () => void; lang: "de" | "en";
}) {
  ...
  const toggle = (a: AllergenKey) => setAllergens((prev) => prev.includes(a) ? prev.filter(x => x !== a) : [...prev, a]);
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-foreground/40 backdrop-blur-sm p-4">
      <form onSubmit={save} className="surface w-full max-w-lg p-6 space-y-4">
        ...
        <div>
          <label className="text-[10px] uppercase tracking-widest text-muted-foreground">{lang === "de" ? "Name" : "Name"}</label>
          <input value={name} onChange={(e) => setName(e.target.value)} required className="mt-1 w-full rounded-lg border border-border bg-card px-3 py-2 text-sm" />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-[10px] uppercase tracking-widest text-muted-foreground">{lang === "de" ? "Kosten €" : "Cost €"}</label>
            <input type="number" step="0.01" value={cost} onChange={(e) => setCost(e.target.value)} className="mt-1 w-full rounded-lg border border-border bg-card px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="text-[10px] uppercase tracking-widest text-muted-foreground">{lang === "de" ? "Preis €" : "Price €"}</label>
            <input type="number" step="0.01" value={price} onChange={(e) => setPrice(e.target.value)} className="mt-1 w-full rounded-lg border border-border bg-card px-3 py-2 text-sm" />
          </div>
        </div>
        <div>
          <label className="text-[10px] uppercase tracking-widest text-muted-foreground">{lang === "de" ? "Allergene" : "Allergens"}</label>
          <div className="mt-2 flex flex-wrap gap-1.5">
            {ALLERGENS.map(a => (
        ...
        <div className="flex justify-end gap-2 pt-2">
          <button type="button" onClick={onClose} className="text-sm px-4 py-2 rounded-full border border-border">{lang === "de" ? "Abbrechen" : "Cancel"}</button>
