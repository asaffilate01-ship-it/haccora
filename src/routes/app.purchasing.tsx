...
interface Row {
  id: string; po_number: string; supplier: string; status: POStatus;
  total_eur: number; expected_date: string | null; line_count: number;
}
...
function PurchasingPage() {
  ...
  const advance = async (r: Row) => {
    const next: POStatus = r.status === "draft" ? "sent" : r.status === "sent" ? "partial" : r.status === "partial" ? "received" : r.status;
  ...
  const open = rows.filter(r => r.status === "sent" || r.status === "partial" || r.status === "draft").length;
  ...
  return (
    <div className="p-6 md:p-10 space-y-8">
      ...
      <div className="grid md:grid-cols-4 gap-4">
        ...
        <Kpi label={t("Heute Anlieferung", "Deliveries today")} value={String(dueToday)} tone="info" />
        <Kpi label={t("Ausgaben", "Spend")} value={`€ ${spend.toFixed(0)}`} />
        <Kpi label={t("Annahmequote", "Acceptance rate")} value={`${Math.round(received/total*100)}%`} tone="success" />
      </div>
        ...
        <div className="surface overflow-hidden">
          {loading ? (
            ...
            <div className="p-10 text-center text-sm text-muted-foreground">
              <ShoppingCart size={20} className="inline opacity-40 mr-2" />{t("Keine Bestellungen.", "No purchase orders yet.")}
            </div>
          ) : (
            <>
                ...
                {rows.map((po) => (
                  <li key={po.id} className="grid grid-cols-1 md:grid-cols-12 items-center px-5 py-3 text-sm gap-2">
                    <div className="md:col-span-2 flex items-center gap-2 font-mono text-xs">
                      <ShoppingCart size={14} className="text-primary" />{po.po_number}
                    </div>
                    <div className="md:col-span-3 flex items-center gap-2"><Truck size={14} className="text-muted-foreground" />{po.supplier}</div>
                    <div className="md:col-span-2 text-xs text-muted-foreground font-mono">{po.expected_date ?? "—"}</div>
                    <div className="md:col-span-1 text-right text-xs font-mono">{po.line_count}</div>
                    <div className="md:col-span-2 text-right font-mono text-xs">€ {Number(po.total_eur).toFixed(2)}</div>
                    <div className="md:col-span-2 text-right flex items-center justify-end gap-2">
                      ...
                      {canManage && po.status !== "received" && po.status !== "rejected" && (
                        <button onClick={() => advance(po)} className="text-[11px] font-semibold text-primary">→</button>
                      )}
                      {canManage && (po.status === "sent" || po.status === "partial") && (
                        <button onClick={() => reject(po)} className="text-[11px] font-semibold text-destructive">✕</button>
                      )}
                      {canManage && (
                        <button onClick={() => remove(po.id)} className="text-muted-foreground hover:text-destructive">
                          <Trash2 size={12} />
                        </button>
                      )}
                    </div>
      ...
      {showForm && canManage && <POForm onClose={() => setShowForm(false)} onSaved={() => { setShowForm(false); load(); }} t={t} />}
    </div>
  );
}

function POForm({ onClose, onSaved, t }: { onClose: () => void; onSaved: () => void; t: (de: string, en: string) => string }) {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    const po_number = `PO-${new Date().getFullYear()}-${Math.floor(Math.random()*10000).toString().padStart(4,"0")}`;
    await supabase.from("purchase_orders").insert({
      po_number, supplier, total_eur: Number(total) || 0, line_count: Number(lines) || 0,
      expected_date: eta || null, status: "draft", created_by: user?.id,
    });
    setSaving(false);
    onSaved();
  };
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-foreground/40 backdrop-blur-sm p-4">
      <form onSubmit={save} className="surface w-full max-w-md p-6 space-y-3">
        ...
        <Field label={t("Lieferant","Supplier")}><input required value={supplier} onChange={e=>setSupplier(e.target.value)} className="mt-1 w-full rounded-lg border border-border bg-card px-3 py-2 text-sm" /></Field>
        <div className="grid grid-cols-2 gap-3">
          <Field label={t("Summe €","Total €")}><input type="number" step="0.01" value={total} onChange={e=>setTotal(e.target.value)} className="mt-1 w-full rounded-lg border border-border bg-card px-3 py-2 text-sm" /></Field>
          <Field label={t("Zeilen","Lines")}><input type="number" value={lines} onChange={e=>setLines(e.target.value)} className="mt-1 w-full rounded-lg border border-border bg-card px-3 py-2 text-sm" /></Field>
        </div>
        <Field label="ETA"><input type="date" value={eta} onChange={e=>setEta(e.target.value)} className="mt-1 w-full rounded-lg border border-border bg-card px-3 py-2 text-sm" /></Field>
